...
// The shared item ledger enforces delivery clearance for every payout entry point.
export const creditVendorWalletsForOrder = async (orderId: string, options?: { paymentReference?: string; provider?: string; source?: string }) => {
  const { settleOrder } = await import('./after-sales');
  return settleOrder(orderId);
};
export const releaseEscrowForOrder = creditVendorWalletsForOrder;
