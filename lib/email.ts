...
class EmailService {
   ...
   */
  async sendEmailVerificationCode({ email, name, code }: { email: string, name: string, code: string }): Promise<boolean> {
      ...
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #ffffff;">
        ...
        <div style="background-color: oklch(0.976 0 0); padding: 25px; border-radius: 10px; margin-bottom: 25px; border: 1px solid oklch(0.898 0 0);">
          ...
          <p style="color: oklch(0.631 0 0); margin: 0 0 20px 0; line-height: 1.6;">
            Enter the code below on the verification page. This code will expire in 15 minutes for security reasons.
          </p>
          <div style="text-align: center; margin: 25px 0;">
            <span style="display: inline-block; background: linear-gradient(135deg, oklch(0.35 0.15 15) 0%, oklch(0.45 0.18 20) 100%); color: white; padding: 18px 40px; border-radius: 8px; font-weight: 700; font-size: 2rem; letter-spacing: 6px; font-family: 'Courier New', Courier, monospace;">
              ${code}
            </span>
          </div>
        </div>
        ...
        <div style="text-align: center; color: oklch(0.631 0 0); font-size: 14px; padding-top: 20px; border-top: 1px solid oklch(0.898 0 0);">
          <p style="margin: 0 0 10px 0;">Having trouble? We're here to help!</p>
          <p style="margin: 0;">Contact us at <a href="mailto:noreply@makeitsell.org" style="color: oklch(0.35 0.15 15); text-decoration: none; font-weight: 600;">noreply@makeitsell.org</a></p>
        </div>
        <div style="text-align: center; color: oklch(0.631 0 0); font-size: 12px; margin-top: 30px; padding-top: 20px; border-top: 1px solid oklch(0.898 0 0);">
          <p style="margin: 0;">This email was sent to ${email}. If you didn't create an account, you can safely ignore this email.</p>
        </div>
      </div>
    `;
    return await this.sendEmail({
      to: email,
      subject: 'Your Make It Sell Verification Code',
      html: emailHtml
    });
  }
