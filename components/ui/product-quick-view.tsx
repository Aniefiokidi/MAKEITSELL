        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 pt-6">
          ...
          <div className="space-y-3 md:space-y-4 order-1 md:order-1">
            ...
            {(product.images || []).length > 1 && (
              <div className="flex gap-2 md:gap-3 overflow-x-auto pb-2 md:pb-0 snap-x snap-mandatory scroll-smooth -mx-0.5 px-0.5">
                {(product.images || []).slice(0, 5).map((img, i) => (
                  <button
                    key={i}
                    onClick={() => setMainImage(img)}
                    className={`shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-lg overflow-hidden border-2 transition-all cursor-pointer snap-center active:scale-95 md:active:scale-100 ${
          ...
          <div className="space-y-4 md:space-y-6 order-1 md:order-2">
            ...
            <div className="flex gap-2 md:gap-3">
              <Button
                className="flex-1 h-10 md:h-12 bg-white hover:bg-accent/10 text-accent border-2 border-accent font-semibold rounded-lg md:rounded-xl transition-all hover:shadow-lg cursor-pointer text-xs md:text-sm active:scale-95 md:active:scale-100"
                onClick={() => {
                  onAddToCart(product)
                  onClose()
                }}
              ...
              >
                <img src="/images/logo3.png" alt="Cart" className="w-6 md:w-8 h-6 md:h-8 -mr-1 md:-mr-2 -mt-2 md:-mt-3" />
                <span className="hidden sm:inline">{product.stock === 0 ? "Out of Stock" : "Add to Cart"}</span>
                <span className="inline sm:hidden">{product.stock === 0 ? "OOS" : "Add"}</span>
              </Button>
              <Button
                variant="outline"
                className="h-10 md:h-12 px-3 md:px-4 rounded-lg md:rounded-xl border-2 hover:bg-accent/10 hover:border-accent transition-all cursor-pointer active:scale-95 md:active:scale-100"
            ...
            {/* Vendor Info */}
            <div className="bg-slate-50 rounded-lg md:rounded-xl p-3 md:p-4 text-xs md:text-sm">
