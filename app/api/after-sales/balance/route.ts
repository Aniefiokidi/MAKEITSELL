...
import { initialLines, activeCase } from '@/lib/after-sales-policy'
...
export async function GET(request: NextRequest) {
 ...
 try {
  ...
  const totals={inEscrow:0,pending:0,onHold:0,available:Number(wallet?.walletBalance||0)}
  ...
  for await(const order of cursor as any){
   const lines=order.protectionLines?.length?order.protectionLines:initialLines(order)
   for(const l of lines){if(l.vendorId!==user.id||l.cancelled||l.settledAt||(storeId&&l.storeId!==storeId))continue
